...
public class DampedItemMeanModelProvider implements Provider<ItemMeanModel> {
    ...
    private static class GlobalRatings {
        private long count;
        private double ratings;

        private double mean() {
           return count > 0 ? ratings/count : 0;
        }
    }
    ...
    public ItemMeanModel get() {
        GlobalRatings globalRatings = new GlobalRatings();
        return MeanModelProvider.getMeanModel(dao, (count, total) -> (total+damping*globalRatings.mean())/(count+damping),
                r -> { globalRatings.count++; globalRatings.ratings+= r.getValue(); });
    }
}
